...
import { ExecutionView as ExecutionViewType, PromptRun, ToolLog } from '../types';
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">{formattedCreatedAt} • {workflowType} • {modelName}</p>
        </div>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-medium flex items-center">
              Project Context
              <span className="text-xs text-gray-500 ml-2 font-normal">(ID: {project.id})</span>
            </CardTitle>
            <CardDescription>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-4">
          ...
          <TabsTrigger value="tool-logs" disabled={!isMCPExecution}>Tool Logs ({toolLogs.length})</TabsTrigger>
