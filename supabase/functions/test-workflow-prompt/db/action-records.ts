...
export async function createActionRecord(
  ...
  try {
    ...
    // Handle SET_FUTURE_REMINDER action type specially
      ...
      return await handleNoActionWithReminder(supabase, promptRunId, projectId, actionData);
    } else {
...
async function handleActionNeeded(
  ...
  if (actionType === "message") {
    return await createMessageAction(supabase, promptRunId, projectId, actionData);
  } else {
    return await createGenericAction(supabase, promptRunId, projectId, actionType, actionData);
  }
}
...
async function createGenericAction(
  supabase: SupabaseClient,
  promptRunId: string,
  projectId: string,
  actionType: string,
...
async function handleNoActionWithReminder(
  supabase: SupabaseClient,
  promptRunId: string,
  projectId: string,
  actionData: any
